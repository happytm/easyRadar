// Code at https://www.electroniclinic.com/rd-03d-mmwave-radar-multi-human-tracking-with-distance-speed-positioning/
// Video at https://www.youtube.com/watch?v=cSI9vedf870
...
// Send single-target detection command
    Serial2.write(Single_Target_Detection_CMD, sizeof(Single_Target_Detection_CMD));
    Serial.println("Single-target detection mode activated.");

// Send single-target detection command
   // Serial2.write(Multi_Target_Detection_CMD, sizeof(Single_Target_Detection_CMD));
  ...
  */
     ...
     */
       // target1_x = (RX_BUF[4] | (RX_BUF[5] << 8)) - 0x200;
        target1_x = (RX_BUF[4] | (RX_BUF[5] << 8));
        target1_x = target1_x & 0x7fff;
    //    if(!(target1_x & 0x8000)){target1_x = target1_x * -1;}

       // target1_y = (RX_BUF[6] | (RX_BUF[7] << 8)) - 0x8000;
        target1_y = (RX_BUF[6] | (RX_BUF[7] << 8));
        target1_y = target1_y & 0x7fff;
     //   if(!(target1_y & 0x8000)){target1_y = target1_y * -1;}
 
        
      //  target1_speed = (RX_BUF[8] | (RX_BUF[9] << 8)) - 0x10;
        target1_speed = (RX_BUF[8] | (RX_BUF[9] << 8));
        target1_speed = target1_speed & 0x7fff;
        if(!(target1_speed & 0x8000)){target1_speed = target1_speed * -1;}
        
        target1_distance = (RX_BUF[10] | (RX_BUF[11] << 8));
        target1_distance = target1_speed & 0x7fff;

       // target1_distance = sqrt(target1_x * target1_x + target1_y * target1_y);
